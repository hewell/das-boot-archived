...
// http://caniuse.com/#search=transition
  ...
  function applyGeneratedPreparationClasses(element, event, options) {
    ...
    if (options.addClass) {
      classes = concatWithSpace(classes,
...
// "OR" statement then it would not be enough to catch that.
    ...
    this.$get = ['$window', '$$jqLite', '$$AnimateRunner', '$timeout',
      ...
      function ($window, $$jqLite, $$AnimateRunner, $timeout,
        ...
        function gcsHashFn(node, extraClasses) {
          ...
          return parentID + '-' + node.getAttribute('class') + '-'
              + extraClasses;
        }
        ...
        function computeCachedCssStaggerStyles(node, className, cacheKey,
          ...
          if (gcsLookup.count(cacheKey) > 0) {
            ...
            if (!stagger) {
              ...
              stagger.animationDuration = Math.max(stagger.animationDuration,
                  0);
              stagger.transitionDuration = Math.max(stagger.transitionDuration,
                  0);
        ...
        return function init(element, options) {
          ...
          var activeClasses = pendClasses(preparationClasses,
              ACTIVE_CLASS_SUFFIX);
          var hasToStyles = styles.to && Object.keys(styles.to).length > 0;
          var containsKeyframeAnimation = (options.keyframeStyle || '').length
              > 0;
          ...
          function start() {
            ...
            var maxStagger = itemIndex > 0
                && ((timings.transitionDuration && stagger.transitionDuration
                === 0) ||
            ...
            function triggerAnimationStart() {
              ...
              startTime = Date.now();
              var timerTime = maxDelayTime + CLOSING_TIME_BUFFER
                  * maxDurationTime;
  ...
  var $$AnimateCssDriverProvider = ['$$animationProvider',
    function ($$animationProvider) {
      ...
      this.$get = ['$animateCss', '$rootScope', '$$AnimateRunner',
    ...
    this.$get = ['$injector', '$$AnimateRunner', '$$jqLite',
      function ($injector, $$AnimateRunner, $$jqLite) {
        ...
        return function (element, event, classes, options) {
          ...
          function packageAnimations(element, event, options, animations,
              fnName) {
            ...
            return function startAnimation(callback) {
              ...
              runners.length ? $$AnimateRunner.all(runners, callback)
                  : callback();
        ...
        function ($$rAF, $rootScope, $rootElement, $document, $$HashMap,
          ...
          return {
            ...
            pin: function (element, parentElement) {
              ...
              assertArg(isElement(parentElement), 'parentElement',
                  'not an element');
          ...
          function queueAnimation(element, event, options) {
            ...
            var className = [node.className, options.addClass,
            ...
            var existingAnimation = (!skipAnimations
                && activeAnimationsLookup.get(node)) || {};
            var hasExistingAnimation = !!existingAnimation.state;

            // there is no point in traversing the same collection of parent ancestors if a followup
            // animation will be run on the same element that already did all that checking work
            if (!skipAnimations && (!hasExistingAnimation
            ...
            if (hasExistingAnimation) {
              var skipAnimationFlag = isAllowed('skip', element, newAnimation,
                  existingAnimation);
              if (skipAnimationFlag) {
                if (existingAnimation.state === RUNNING_STATE) {
                  ...
                  mergeAnimationOptions(element, existingAnimation.options,
                      options);
                  return existingAnimation.runner;
                }
              }

              var cancelAnimationFlag = isAllowed('cancel', element,
                  newAnimation, existingAnimation);
              if (cancelAnimationFlag) {
                ...
                var joinAnimationFlag = isAllowed('join', element, newAnimation,
                    existingAnimation);
                if (joinAnimationFlag) {
                  if (existingAnimation.state === RUNNING_STATE) {
                    normalizeAnimationOptions(element, options);
                  } else {
                    applyGeneratedPreparationClasses(element,
                        isStructural ? event : null, options);

                    event = newAnimation.event = existingAnimation.event;
                    options = mergeAnimationOptions(element,
                        existingAnimation.options, newAnimation.options);
            ...
            $rootScope.$$postDigest(function () {
              ...
              if (animationCancelled || animationDetails.counter !== counter
                ...
                if (animationCancelled || (isStructural
              ...
              var realRunner = $$animation(element, event,
          ...
          function areAnimationsAllowed(element, parentElement, event) {
            ...
            while (parentElement && parentElement.length) {
              if (!rootElementDetected) {
                ...
                rootElementDetected = isMatchingElement(parentElement,
                    $rootElement);
              }
              ...
              if (!bodyElementDetected) {
                ...
                bodyElementDetected = isMatchingElement(parentElement,
                    bodyElement);
              }
            ...
            forEach(groupedAnimations, function (animationEntry) {
              toBeSortedAnimations.push({
                domNode: getDomNode(animationEntry.from
                    ? animationEntry.from.element : animationEntry.element),
            ...
            forEach(refLookup, function (operations, key) {
              ...
              if (!anchorGroups[lookupKey]) {
                var group = anchorGroups[lookupKey] = {
                  ...
                  classes: cssClassesIntersection(fromAnimation.classes,
                      toAnimation.classes),
                  from: fromAnimation,
                  to: toAnimation,
